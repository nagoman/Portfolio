...
This version uses React components loaded in the browser, so **Node.js and npm are not required**. Visitors open the normal website URL, so they do **not** need to type an `.html` extension.
...
> If you see `bash: npm: command not found`, that is okay. This project does not need npm. Use `./run.sh` or `python3 -m http.server 8000` instead.
